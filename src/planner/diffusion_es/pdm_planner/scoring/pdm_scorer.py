...
from src.planner.diffusion_es.pdm_planner.observation.pdm_occupancy_map import (
    PDMOccupancyMap,
)
from src.planner.diffusion_es.pdm_planner.scoring.pdm_comfort_metrics import (
    ego_is_comfortable,
)
from src.planner.diffusion_es.pdm_planner.scoring.pdm_scorer_utils import (
    get_collision_type,
)
...
class PDMScorer:
    ...
    def __init__(
        self, proposal_sampling: TrajectorySampling, scorer_config, comfort_config
    ):
        """
        ...
        # Load scorer config
        self._weighted_metrics_weights = np.zeros(
            len(WeightedMetricIndex), dtype=np.float64
        )
        self._weighted_metrics_weights[WeightedMetricIndex.PROGRESS] = scorer_config[
            "weighted_metrics"
        ]["progress"]
        self._weighted_metrics_weights[WeightedMetricIndex.TTC] = scorer_config[
            "weighted_metrics"
        ]["ttc"]
        self._weighted_metrics_weights[WeightedMetricIndex.COMFORTABLE] = scorer_config[
            "weighted_metrics"
        ]["comfortable"]
        self._weighted_metrics_weights[WeightedMetricIndex.LANE_FOLLOWING] = (
            scorer_config["weighted_metrics"]["lane_following"]
        )
        self._weighted_metrics_weights[WeightedMetricIndex.PROXIMITY] = scorer_config[
            "weighted_metrics"
        ]["proximity"]
    ...
    def _reset(
        ...
        self._ego_map_coords = state_array_to_coords_array(
            ...
            length_width_reduction=self._scorer_config["max_drivable_area_violation"]
            * np.sqrt(2),
        )
        ...
        self._all_collisions = np.ones((3, self._num_proposals), dtype=np.float64)
    ...
    def _calculate_ego_area(self) -> None:
        ...
        coordinates = self._ego_map_coords.reshape(
            n_proposals * n_horizon * n_points, 2
        )
    ...
    def _calculate_no_at_fault_collision(self) -> None:
        ...
        for time_idx in range(self._proposal_sampling.num_poses + 1):
            ...
            for proposal_idx, geometry_idx in zip(intersecting[0], intersecting[1]):
                ...
                tracked_object_polygon = self._observation[time_idx]._geometries[
                    geometry_idx
                ]
    ...
    def _calculate_progress(self) -> None:
        ...
        centerline = self._centerline.get_nearby_path(
            self._initial_ego_state, self._speed_limit_mps
        )
        ...
        for proposal_idx in range(self._num_proposals):
            ...
            end_point = Point(
                *self._ego_map_coords[proposal_idx, -1, BBCoordsIndex.CENTER]
            )
    ...
    def _calculate_driving_direction_compliance(self) -> None:
        ...
        for proposal_idx in range(self._num_proposals):
            ...
            if (
                max_oncoming_traffic_progress
                < self._scorer_config["driving_direction_compliance_threshold"]
            ):
                driving_direction_compliance_scores[proposal_idx] = 1.0
            elif (
                max_oncoming_traffic_progress
                < self._scorer_config["driving_direction_violation_threshold"]
            ):
    ...
    def _calculate_speed_limit_compliance(self):
        ...
        if self._speed_limit_mps is not None:
            velocity_x, velocity_y = (
                self._states[:, :, StateIndex._VELOCITY_X],
                self._states[:, :, StateIndex._VELOCITY_Y],
            )
            speeds = np.sqrt(velocity_x**2 + velocity_y**2)

            speeds_over_limit = np.clip(speeds - self._speed_limit_mps, 0.0, None)

            dt_in_sec = 0.1  # np.mean(np.diff(time_series.time_stamps)) * 1e-6
            scenario_duration_in_sec = (
                4.0  # (time_series.time_stamps[-1] - time_series.time_stamps[0]) * 1e-6
            )
            violation_loss = (
                np.sum(speeds_over_limit, axis=1)
                * dt_in_sec
                / (
                    self._scorer_config["max_overspeed_value_threshold"]
                    * scenario_duration_in_sec
                )
            )
    ...
    def _calculate_lane_following(self):
        ...
        for token in self._route_lane_dict:
            if token in self._drivable_area_map.tokens:
                ...
                lane = self._map_api._get_lane(
                    token
                ) or self._map_api._get_lane_connector(token)
        ...
        for centerline in centerlines:
            ...
            xy_loss = np.linalg.norm(
                trajectories[:, None] - centerline[None, :, None, :2], axis=-1
            )
            heading_loss = -np.cos(
                centerline_heading[None, :, None] - trajectory_heading[:, None]
            )
            ...
            heading_loss = np.take_along_axis(
                heading_loss, closest_idx[:, None], axis=1
            ).squeeze(1)

            # Convert losses to scores
            max_lane_deviation = self._scorer_config["max_lane_deviation"]
            position_score = (
                np.clip(max_lane_deviation - xy_loss, 0, max_lane_deviation)
                / max_lane_deviation
            )
    ...
    def _calculate_proximity(self):
        ...
        """
        self._weighted_metrics[WeightedMetricIndex.PROXIMITY] = np.ones(
            self._num_proposals, dtype=np.float64
        )
        if self._lead_agent is not None:
            ...
            score = np.clip(
                min_dists / self._scorer_config["min_distance_to_lead"], 0, 1
            )
    ...
    def remove_yield_metric(self):
        if OptionalMetricIndex.YIELD in self._optional_metric_ids:
            self._optional_metric_ids.pop(
                self._optional_metric_ids.index(OptionalMetricIndex.YIELD)
            )
    ...
    def _calculate_yield_metric(self):
        ...
        if OptionalMetricIndex.YIELD in self._optional_metric_ids:
            ...
            coords_exterior = np.stack(
                self._observation[0][self._target_id].exterior.xy, axis=-1
            )
            ...
            for idx, future_time_idx in enumerate(future_time_idcs):
                delta_t = (
                    float(future_time_idx) * self._proposal_sampling.interval_length
                )
                coords_exterior_time_steps[idx] = (
                    coords_exterior_time_steps[idx] + dxy_per_s[None] * delta_t
                )
            ...
            for proposal_idx in range(self._num_proposals):
                intersecting = target_tree.query(
                    self._ego_polygons[proposal_idx], predicate="intersects"
                )
    ...
    def remove_lane_metric(self):
        if OptionalMetricIndex.LANE in self._optional_metric_ids:
            self._optional_metric_ids.pop(
                self._optional_metric_ids.index(OptionalMetricIndex.LANE)
            )
        self._lane_goal = None
    ...
    def _calculate_lane_metric(self):
        if OptionalMetricIndex.LANE in self._optional_metric_ids:
            ...
            dists = np.linalg.norm(
                trajectory[:, :, None] - self._lane_goal[None, None, :, :2], axis=-1
            )
            ...
            scores = (
                MAX_LANE_DIST - np.clip(dists, 0.0, MAX_LANE_DIST)
            ) / MAX_LANE_DIST
    ...
    def remove_soft_speed_metric(self):
        if OptionalMetricIndex.SOFT_SPEED in self._optional_metric_ids:
            self._optional_metric_ids.pop(
                self._optional_metric_ids.index(OptionalMetricIndex.SOFT_SPEED)
            )
