...
from src.planner.diffusion_es.pdm_planner.abstract_pdm_planner import (
    AbstractPDMPlanner,
)
from src.planner.diffusion_es.pdm_planner.observation.pdm_observation import (
    PDMObservation,
)
from src.planner.diffusion_es.pdm_planner.proposal.batch_idm_policy import (
    BatchIDMPolicy,
)
from src.planner.diffusion_es.pdm_planner.proposal.pdm_generator import (
    PDMGenerator,
)
from src.planner.diffusion_es.pdm_planner.proposal.pdm_proposal import (
    PDMProposalManager,
)

if os.environ.get("CLOSED", 0):
    from src.planner.diffusion_es.pdm_planner.scoring.pdm_scorer_old import (
        PDMScorer,
    )
else:
    from src.planner.diffusion_es.pdm_planner.scoring.pdm_scorer import (
        PDMScorer,
    )
from src.planner.diffusion_es.pdm_planner.simulation.pdm_simulator import (
    PDMSimulator,
)
from src.planner.diffusion_es.pdm_planner.utils.pdm_emergency_brake import (
    PDMEmergencyBrake,
)
from src.planner.diffusion_es.pdm_planner.utils.pdm_geometry_utils import (
    parallel_discrete_path,
)
from src.planner.diffusion_es.pdm_planner.utils.pdm_path import PDMPath
from src.planner.diffusion_es.pdm_planner.observation.pdm_observation_utils import (
    get_drivable_area_map,
)
from src.planner.diffusion_es.pdm_planner.utils.pdm_array_representation import (
    state_array_to_ego_states,
    ego_states_to_state_array,
)
from src.planner.diffusion_es.pdm_planner.utils.pdm_enums import (
    MultiMetricIndex,
    WeightedMetricIndex,
    BBCoordsIndex,
)
from src.planner.diffusion_es.pdm_planner.utils.lane_graph_parser import (
    parse_lane_graph,
)
from src.planner.diffusion_es.pdm_planner.utils.diffusion_utils import (
...
EgoState = DummyType
TimePoint = DummyType
LaneGraphEdgeMapObject = DummyType
NuPlanLane = DummyType
PlannerInput = DummyType
InterpolatedTrajectory = DummyType
TrajectorySampling = DummyType
ModelLoader = DummyType
build_torch_module_wrapper = DummyType
LightningModuleWrapper = DummyType
Trajectory = DummyType
visualize = DummyType
SemanticMapLayer = DummyType
# -------------------------------------
...
class PDMDiffusionPlanner(AbstractPDMPlanner):
    ...
    def __init__(
        ...
        self._observation = PDMObservation(
            trajectory_sampling,
            trajectory_sampling
            if not os.environ.get("CLOSED", 0)
            else proposal_sampling,
            map_radius,
        )
    ...
    def compute_planner_trajectory(self, current_input: PlannerInput):
        ...
        self._drivable_area_map = get_drivable_area_map(
            self._map_api, self.ego_state, self._map_radius, self._route_lane_dict
        )
    ...
    def _update_proposal_manager(self, ego_state: EgoState):
        ...
        if isinstance(self._current_lane, NuPlanLane):
            ...
            edges = (
                self._current_lane.incoming_edges
                + self._current_lane.outgoing_edges
                + [self._current_lane]
            )
            speed_limits = [
    ...
    def _get_closed_loop_trajectory(
        ...
        if self._iteration % self._replan_freq == 0:
            ...
            pdm_proposals = torch.from_numpy(pdm_proposals).to(
                next(self._model_loader._model.parameters()).device
            )
            ...
            if self._use_pdm_proposals:
                predictions = self._model_loader._model.run_diffusion_es(
                    features, warm_start=pdm_proposals
                )
            else:
            ...
            _trajectory_tensor = trajectory_predicted.data

            all_trajectories = (
                predictions["multimodal_trajectories"].detach().cpu().numpy()
            )
            _all_trajectories = all_trajectories.reshape(-1, 16, 3)
            ...
            if os.environ.get("DEBUG", 0):
                ...
                if (
                    self._time_idxs_to_save is not None
                    and self._iteration in self._time_idxs_to_save
                ):
    ...
    def _generate_constraints(self, current_input):
        ...
        if self._follow_centerline:
            centerline = self._centerline.get_nearby_path(
                self.ego_state, self._speed_limit
            )
            centerline = np.array(centerline.coords)
            self._scorer.add_lane_metric(centerline)

        return [self._make_pdm_scorer_constraint(current_input, weight=100.0)]
        ...
        def constraint_fn(trajectory):
            ...
            trajectory_padded = np.concatenate(
                [
                    trajectory_global,
                    np.zeros(
                        (trajectory_global.shape[0], trajectory_global.shape[1], 8)
                    ),
                ],
                axis=2,
            )
            ...
            # Simulate
            trajectory_sim = self._simulator.simulate_proposals(
                trajectory_padded, self.ego_state
            )
            trajectory_sim_local = convert_to_local(
                self.ego_state, trajectory_sim[..., :3]
            )  # for viz
    ...
    def _save_gif(self, fname="", frames=None, duration=200):
        ...
        frames[0].save(
            fname, save_all=True, append_images=frames[1:], duration=duration, loop=0
        )
    ...
    def _visualize(self, ego_state, features, predictions):
        ...
        centerline = self._centerline.get_nearby_path(
            self.ego_state, speed_limit=self._speed_limit
        )
        centerline = np.array(centerline.coords)
    ...
    def _visualize_paper(self, ego_state, features, predictions, save_path):
        ...
        cv2.imwrite(
            f"/zfsauton/datasets/ArgoRL/brianyan/nuplan_exp/viz/{save_path}",
            frame[..., ::-1],
        )
    ...
    def _visualize_collisions(self, fname, scores):
        ...
        _mask3 = (active_lateral_scores == 0) * (~mask1) * (~mask2)
    ...
    def _visualize_drivable_area(self):
        ...
        for traj_idx in range(ego_trajectories.shape[0]):
            plt.plot(
                ego_trajectories[traj_idx, :, 0],
                ego_trajectories[traj_idx, :, 1],
                c=colors[traj_idx],
            )
    ...
    def _visualize_driving_direction(self):
        ...
        for traj_idx in range(ego_trajectories.shape[0]):
            ...
            oncoming_traffic_masks = (
                self._scorer._ego_areas[traj_idx, :, 2].flatten().astype(float)
            )
    ...
    def _visualize_ids(self, ego_state, features):
        ...
        for object_type in object_manager._dynamic_object_coords.keys():
            ...
            if len(object_manager._dynamic_object_coords[object_type]) > 0:
                coords = np.stack(
                    object_manager._dynamic_object_coords[object_type], axis=0
                )
                ...
                headings = [
                    object_manager.unique_objects[token].center.heading
                    for token in tokens
                ]
        ...
        for type_name in dynamic_coords_by_type:
            for i, coords in enumerate(dynamic_coords_by_type[type_name]):
                ...
                print(
                    f"{type_name} {i} at ({center[0]},{center[1]}) at heading {heading:.02f}"
                )
        ...
        ]  # self._map_api.get_available_map_objects()
        map_objects = self._map_api.get_proximal_map_objects(
            ego_state.center.point, 25.0, layers=map_object_types
        )
        ...
        for i, node in enumerate(map_objects):
            ...
            print(
                f"LANE {i} at ({centerline[0,0]},{centerline[0,1]}) to ({centerline[-1,0]},{centerline[-1,1]}) with heading {node.heading:.2f}"
            )
