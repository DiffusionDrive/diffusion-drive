...
def parse_lane_graph(nodes):
    ...
    for node in nodes:
        node.incoming = [
            lane_object_to_node[obj.id]
            for obj in node.lane_object.incoming_edges
            if obj.id in lane_object_to_node
        ]
        node.outgoing = [
            lane_object_to_node[obj.id]
            for obj in node.lane_object.outgoing_edges
            if obj.id in lane_object_to_node
        ]
        node.adjacent = [
            lane_object_to_node[obj.id]
        ...
        if len(node.incoming) == 1:
            ...
            if len(incoming_node.outgoing) == 1:
                ...
                incoming_node.centerline = np.concatenate(
                    [incoming_node.centerline, node.centerline], axis=0
                )
                continue
        ...
        if len(node.outgoing) == 1:
            ...
            if len(outgoing_node.incoming) == 1:
                ...
                outgoing_node.centerline = np.concatenate(
                    [node.centerline, outgoing_node.centerline], axis=0
                )
                continue
