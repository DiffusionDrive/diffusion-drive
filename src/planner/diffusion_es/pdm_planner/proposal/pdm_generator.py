...
class PDMGenerator:
    ...
    def _initialize_states(self, lateral_batch_idcs: List[int]) -> None:
        ...
        self._state_idm_array[lateral_batch_idcs, 0, StateIDMIndex.PROGRESS] = (
            ego_progress
        )
        self._state_idm_array[lateral_batch_idcs, 0, StateIDMIndex.VELOCITY] = (
            ego_velocity
        )
    ...
    def _update_states_se2(self, lateral_batch_idcs: List[int], time_idx: int) -> None:
        ...
        self._state_array[lateral_batch_idcs, time_idx, StateIndex.STATE_SE2] = (
            states_se2_array
        )
                ...
                if len(agents_ahead) > 0:  # red light, object or agent ahead
                    ...
                    if time_idx == 2:
                        ...
                        if nearest_agent in self._observation.unique_objects:
                            agent_object = self._observation.unique_objects[
                                nearest_agent
                            ]
                            if (
                                relative_distances[argmin] < self._lead_agent_dist
                            ) and (agent_object.tracked_object_type in AGENT_TYPES):
                    ...
                    if self._observation.red_light_token not in nearest_agent:
                        leading_agent_array[LeadingAgentIndex.VELOCITY] = (
    ...
    def dist_to_leading_vehicle(self):
        ...
        lead_rear_length = self._leading_agent_array[
            :, 2, LeadingAgentIndex.LENGTH_REAR
        ]
