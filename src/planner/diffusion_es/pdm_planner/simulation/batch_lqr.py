...
from src.planner.diffusion_es.pdm_planner.simulation.batch_lqr_utils import (
...
class BatchLQRTracker:
    ...
    def _compute_reference_velocity_and_curvature_profile(
        ...
        if reference_length < self._tracking_horizon:
            reference_curvature_profiles[:, reference_length:] = (
    ...
    def _lateral_lqr_controller(
        ...
        I_mat: npt.NDArray[np.float64] = np.eye(n_lateral_states, dtype=np.float64)
        ...
        states_matrix_at_step: npt.NDArray[np.float64] = np.tile(
            I_mat[None, None, ...], [self._tracking_horizon, batch_dim, 1, 1]
        )  # (horizon, batch, 3, 3)
