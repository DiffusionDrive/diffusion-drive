...
from src.planner.diffusion_es.pdm_planner.observation.pdm_object_manager import (
    PDMObjectManager,
)
            ...
            if has_dynamic_object:
                ...
                for ratio in const_vel_ratios:
                    ...
                    dynamic_object_polygons.append(
                        shapely.creation.polygons(dynamic_object_coords_t)
                    )
                dynamic_object_polygons = np.concatenate(
                    dynamic_object_polygons, axis=0
                )
                dynamic_object_tokens_t = list(
                    chain(
                        *[dynamic_object_tokens for _ in range(len(const_vel_ratios))]
                    )
                )
            else:
