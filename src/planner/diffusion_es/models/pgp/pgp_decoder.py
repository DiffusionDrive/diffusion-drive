...
from src.planner.diffusion_es.models.pgp.utils import cluster_traj
...
class LVM(nn.Module):
    def __init__(
        ...
        if agg_type not in ["combined", "sample_specific"]:
            raise ValueError(
                f"argument agg_type has to be either 'combined' or 'sample_specific' but {agg_type} was given"
            )
