...
ProgressStateSE2 = DummyType
InterpolatedPath = DummyType
EgoState = DummyType
StateVector2D = DummyType
DetectionsTracks = DummyType


def get_pacifica_parameters(*args, **kwargs):
    return None
...
def smooth_centerline_trajectory(trajectories: torch.Tensor):
    ...
    assert torch.all(torch.isfinite(smoothed_trajectories)), f"""
