...
class PGPGraphMap:
    ...
    def to_feature_tensor(self) -> PGPGraphMap:
        ...
        """
        return PGPGraphMap(
            lane_node_feats=torch.as_tensor(self.lane_node_feats),
            lane_node_masks=torch.as_tensor(self.lane_node_masks),
            lane_ids=torch.as_tensor(self.lane_ids),
            s_next=torch.as_tensor(self.s_next),
            edge_type=torch.as_tensor(self.edge_type),
            edge_on_route_mask=torch.as_tensor(self.edge_on_route_mask),
            nodes_on_route_flag=torch.as_tensor(self.nodes_on_route_flag),
            red_light_mask=torch.as_tensor(self.red_light_mask),
            red_light_flag=torch.as_tensor(self.red_light_flag),
        )
