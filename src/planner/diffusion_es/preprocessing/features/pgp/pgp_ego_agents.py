...
class PGPEgoAgents:
    ...
    def to_feature_tensor(self) -> PGPEgoAgents:
        ...
        """
        return PGPEgoAgents(
            vehicle_agent_feats=torch.as_tensor(self.vehicle_agent_feats),
            vehicle_agent_masks=torch.as_tensor(self.vehicle_agent_masks),
            pedestrians_agent_feats=torch.as_tensor(self.pedestrians_agent_feats),
            pedestrians_agent_masks=torch.as_tensor(self.pedestrians_agent_masks),
            ego_feats=torch.as_tensor(self.ego_feats),
        )
    ...
    def collate(cls, batch: List[PGPEgoAgents]) -> PGPEgoAgents:
        ...
        for i, item in enumerate(batch):
            ...
            collated_pedestrians_agent_feats[i, :num_peds, :] = (
                item.pedestrians_agent_feats
            )
            collated_pedestrians_agent_masks[i, :num_peds, :] = (
