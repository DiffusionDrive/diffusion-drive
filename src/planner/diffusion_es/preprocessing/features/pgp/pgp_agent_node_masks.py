...
class PGPAgentNodeMasks:
    ...
    def to_feature_tensor(self) -> PGPAgentNodeMasks:
        return PGPAgentNodeMasks(
            vehicle_node_masks=torch.as_tensor(self.vehicle_node_masks),
            pedestrian_node_masks=torch.as_tensor(self.pedestrian_node_masks),
        )

    def to_device(self, device: torch.device) -> PGPAgentNodeMasks:
        # Removed nuPlan's 'validate_type' checks completely
        return PGPAgentNodeMasks(
            vehicle_node_masks=torch.as_tensor(self.vehicle_node_masks).to(
                device=device
            ),
            pedestrian_node_masks=torch.as_tensor(self.pedestrian_node_masks).to(
                device=device
            ),
        )
    ...
    def collate(cls, batch: List[PGPAgentNodeMasks]) -> PGPAgentNodeMasks:
        ...
        for i, item in enumerate(batch):
            ...
            collated_vehicle_node_masks[i, :num_nodes, :num_vehicles] = (
                item.vehicle_node_masks
            )
            collated_pedestrian_node_masks[i, :num_nodes, :num_pedestrians] = (
