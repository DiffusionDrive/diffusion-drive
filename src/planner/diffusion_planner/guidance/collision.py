...
ego_size = [5.18, 2.30]  # CommonRoadVehicle [Length, Width]
...
def center_rect_to_points(rect):
    ...
    rot = torch.stack([cos_h, -sin_h, sin_h, cos_h], dim=1).reshape(
        -1, 2, 2
    )  # [B, 2, 2]
    lw = torch.einsum(
        "bj,ij->bij",
...
def collision_guidance_fn(x, t, cond, inputs, *args, **kwargs) -> torch.Tensor:
    ...
    predictions = torch.cat(
        [ego_pred, neighbors_pred.detach()], dim=1
    )  # [B, P + 1, T, 4]
    ...
    x_aux = torch.autograd.grad(reward.sum(), x, retain_graph=True, allow_unused=True)[
        0
    ][:, 0, :, :2]  # [B, T, 2]
